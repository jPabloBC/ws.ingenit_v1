...
export default function CategoriesPage() {
  ...
  const loadCategories = async () => {
    try {
      setLoading(true);
      const storeType = storeConfig?.type;
  ...
  const loadCategoriesWithoutStoreType = async () => {
    try {
      setLoading(true);
      const categoriesData = await getCategories(); // Sin storeType
      setCategories(categoriesData || []);
    } catch (error) {
      console.error('Error loading categories without storeType:', error);
      toast.error('Error al cargar las categorías');
    } finally {
      setLoading(false);
    }
  ...
  const filteredCategories = categories.filter(category =>
    normalize(category.name).includes(normalize(searchTerm)) ||
    normalize(category.description || '').includes(normalize(searchTerm))
  );
            ...
            {storeConfig && (
              <div className="mt-2">
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                  {storeConfig.name} ({storeConfig.type})
                </span>
              </div>
            )}
          </div>
