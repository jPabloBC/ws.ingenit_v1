-- =====================================================
-- REINICIAR IDs DE ws_categories - VERSIÓN ULTRA SIMPLE
-- =====================================================
...
-- Paso 3: Mostrar el mapeo de IDs
SELECT 
    'Mapeo de IDs' as descripcion,
    c.id as id_viejo,
    t.id as id_nuevo,
    c.name
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ws_products') THEN
        -- Actualizar referencias usando JOIN directo
        FOR old_id, new_id, category_name IN 
            SELECT c.id, t.id, c.name
            FROM ws_categories c
            JOIN ws_categories_temp t ON c.name = t.name
        LOOP
            UPDATE ws_products 
            ...
            RAISE NOTICE 'Actualizado: % (ID % -> %)', category_name, old_id, new_id;
        END LOOP;
...
DROP TABLE IF EXISTS ws_categories CASCADE;
...
SELECT 
    'Categorías con nuevos IDs' as descripcion,
    id, 
    name, 
    store_type 
FROM ws_categories 
